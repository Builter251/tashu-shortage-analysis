...
| 3 | **인코딩이 파일마다 다름** (cp949 9개 / utf-8-sig 11개) | 두 번째 파일에서 `UnicodeDecodeError` | 포털 안내대로 CP949 고정 시 적재 중단 |
...
> **40km는 어디서 나온 수인가.** 문헌값이 아니라 관측 분포의 극단 꼬리에서 잡은 선이다.
> 이용거리 중앙값은 1.0km, 75%가 2.0km, 99%가 12.4km, 99.99%가 25.2km다.
> 40km에는 **상위 0.0008%(154건)** 만 걸린다 — 사실상 "일상적인 이용으로는 설명이 안 되는 거리"의 경계다.
> 물리적 상한으로 교차 확인하면, 대전 갑천 자전거길 왕복이 약 40km 로 알려져 있다.
> 즉 *하루에 자전거로 낼 수 있는 최장 거리*와 *분포의 극단 꼬리*가 같은 자리에 떨어진다.
> 다만 이 154건은 **삭제가 아니라 플래그**이므로, 선을 30km나 50km로 옮겨도 어떤 결론도 움직이지 않는다.
...
### 그림 12 — 임계값을 어디에 그었고, 왜 거기인가

![임계값 보정](images/12_threshold_calibration.png)

**분류 임계값은 문헌값이 아니라 내가 그은 선이다.** 그렇다면 근거를 남겨야 한다.

- **기준선이 있는 값**: CPI는 첨두 6시간 / 24시간 = **0.25** 가 "시간대가 완전히 균등할 때"의 값이고, WR은 비율이므로 **1.00** 이 "주말 = 평일"의 구조적 분기점이다. 이 둘은 정의에서 나오므로 논쟁의 여지가 없다.
- **그러나 기준선으로는 나눌 수 없다**: 판정 대상 1,243개 중 **98.1%가 CPI 0.25를 넘는다**(중앙값 0.375). 대전 타슈는 도시 전체가 출퇴근 쪽으로 기울어 있다. 0.25로 자르면 거의 전부가 "출퇴근형"이 되어 분류가 정보를 주지 못한다. 그래서 **절대 기준이 아니라 상대 기준(분위수)** 이 필요했다.
- **채택선의 실제 위치**: CPI 0.40 은 상위 사분위 **q75 = 0.415**, WR 0.80 은 하위 사분위 **q25 = 0.802** 자리에 떨어졌다. 다만 이것은 **사전에 유도한 값이 아니라 사후에 확인한 일치**다. 설계 시점에는 "0.25의 약 1.6배"라는 어림값으로 그었다. WR의 0.80·1.10 은 1.00 둘레의 **완충대**로, WR 0.99와 1.01을 서로 다른 유형으로 부르지 않기 위한 장치다.
- **선을 흔들어도 결론은 그대로다**: 임계값을 6가지로 바꿔 재계산하면(`data/processed/threshold_sensitivity.csv`), 순유출 상위 30개의 출퇴근형 쏠림배수가 **1.03~1.96** 사이에서만 움직인다. 어느 선에서도 "출퇴근형이면 부족하다"가 성립하지 않는다. 임계값과 무관한 연속 상관 **r = −0.062** 역시 변하지 않는다.
- **남는 한계**: CPI 분포는 단봉이고 뚜렷한 골이 없다. 대여소는 유형으로 나뉘는 집단이라기보다 **연속체**에 가깝고, 어떤 임계값이든 그 연속체를 임의로 자르는 행위다. 그래서 이 리포트는 분류 결과와 **연속 상관을 항상 함께** 보고한다.

재현: `python3 src/08_threshold_calibration.py`
...
![반례](images/11_robustness.png)

왼쪽: 성수기와 비수기의 순유출을 비교하면 **31%의 대여소가 방향이 뒤집힌다.**
오른쪽: 재배치 시각 추정 가정을 바꾸면 "언제 채워지는가"가 완전히 달라진다.
...
### 반례 — 이 결론이 달라질 수 있는 조건

결론을 방어하는 대신, **어떤 선택을 바꾸면 깨지는지** 직접 흔들어 봤다. (`src/07_robustness.py`, 그림 11)

**반례 A — 기간을 바꾸면 순유출 랭킹이 바뀐다**

전체 19개월 대신 성수기(2025-04~09)와 비수기(2025-11~2026-02)로 나눠 다시 계산했다.
(기간이 짧아지면 표본이 줄어드므로 각 기간 대여 300건 이상만 비교)

| 비교 구간 | 공통 대여소 | 순위 상관 | Top10 겹침 | 부호 뒤집힘 |
|---|---|---|---|---|
| 전체 ↔ 성수기 | 1,152 | 0.806 | 7 / 10 | 230개 (20.0%) |
| 전체 ↔ 비수기 | 939 | 0.712 | 6 / 10 | 234개 (24.9%) |
| 성수기 ↔ 비수기 | 892 | — | — | **274개 (30.7%)** |

각 기간의 순유출 1~2위도 다르다.

| 기간 | 1위 | 2위 |
|---|---|---|
| 전체 19개월 | 목동 한사랑아파트 2.76 | 장동 대덕대학교 2.27 |
| 성수기 | 목동 한사랑아파트 3.56 | **도룡동 대전컨벤션센터 2.52** |
| 비수기 | **장동 대덕대학교 3.16** | 목동 한사랑아파트 3.07 |

**해석**: 목동 한사랑아파트가 1위라는 결론은 전체 기간에서만 성립한다.
비수기에는 장동 대덕대학교가 앞선다. 도룡동 대전컨벤션센터는 성수기에만 2위로 올라오는데,
**전시·행사 일정이라는 외부 변수**가 작용했을 가능성이 높다.
그리고 **약 3개 중 1개 대여소는 계절에 따라 순유출↔순유입 방향 자체가 뒤집힌다**
(예: 탄방동 e편한세상 — 성수기 순유출 0.26 → 비수기 순유입 0.21).

> **그래서 무엇을 고쳐 말해야 하나**: "이 대여소가 부족하다"가 아니라
> **"19개월 평균으로는 이 대여소가 가장 부족하다"** 라고 기간을 붙여 말해야 한다.
> 운영에 쓰려면 계절별로 따로 랭킹을 내야 한다.

**반례 B — 집계 단위를 올리면 핵심이 사라진다**

목동 한사랑아파트를 **시간 단위**로 보면 06시 −0.33대에서 시작해 23시 −3.00대까지
하루 안에서 **3.12대 폭**으로 흘러내린다(그림 04). 이걸 **일 단위**로 올리면
"하루 평균 −3.00대" 숫자 하나가 되고, **"언제 비는가"가 통째로 사라진다.**
재배치 시각을 설계하려면 시간 단위가 필수다. 반대로 전체 추세(인사이트 1)를 시간 단위로 보면
노이즈에 묻혀 계절성이 안 보인다 — **질문마다 맞는 집계 단위가 다르다.**

**반례 C — 재배치 시각은 가정이 정한다 (민감도 분석)**

재배치 시각을 "궤적 공백의 중간점"으로 추정했다. 이 가정을 바꿔 봤다.

| 가정 | 유입 피크 시각 | 새벽(0~5시) 비중 |
|---|---|---|
| **공백의 중간점** (현재) | 15시 | **25.5%** |
| 공백 시작 직후 | 18시 | 6.3% |
| 공백 종료 직전 | 21시 | 5.3% |

같은 데이터인데 피크가 15시↔18시↔21시로 움직이고, 새벽 비중은 **25.5% → 5.3%로 5배 차이**가 난다.
→ **그림 04의 주황 선(재배치 반영)이 보여주는 "언제 채워지는가"는 신뢰할 수 없다.**
재배치의 **총량과 방향**(인사이트 5·6)은 어느 가정에서도 변하지 않으므로 그대로 유효하지만,
**시각에 근거한 주장은 하지 않는다.** 실측(OpenAPI)으로만 확정할 수 있다.

**반례 D — 외부 변수가 빠져 있다**

날씨를 넣지 않았다. STL 잔차가 가장 크게 음수인 날들(2025-06-20, 06-21, 07-17)은
강수일일 가능성이 높지만 확인하지 못했다. 기상 데이터를 결합하면
인사이트 1의 "비수기 이용 증가"가 **이용 행태 변화가 아니라 그해 겨울이 따뜻했던 것**으로
설명될 여지도 있다.
