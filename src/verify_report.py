...
# 08_threshold_calibration.py — 임계값 근거 수치 (PLAN 3-1 / REPORT 그림 12 / RUBRIC)
jud = met[met.monthly_rentals >= 30].dropna(subset=["CPI", "WR"])
chk("판정 대상 대여소", 1243, len(jud))
chk("CPI q75", 0.415, jud.CPI.quantile(0.75), 0.001)
chk("CPI 중앙값", 0.375, jud.CPI.quantile(0.50), 0.001)
chk("CPI>0.25 비율%", 98.1, (jud.CPI > 0.25).mean() * 100, 0.05)
chk("WR q25", 0.802, jud.WR.quantile(0.25), 0.001)
chk("WR q90", 1.137, jud.WR.quantile(0.90), 0.001)
chk("CPI-순유출 상관", -0.062, jud.CPI.corr(jud.net_out_per_day), 0.001)
ts = pd.read_csv(P / "threshold_sensitivity.csv")
chk("민감도 시나리오 수", 6, len(ts))
chk("쏠림배수 최소", 1.03, ts["쏠림배수"].min(), 0.005)
chk("쏠림배수 최대", 1.96, ts["쏠림배수"].max(), 0.005)
